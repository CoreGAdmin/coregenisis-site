...
export default function PageFooter() {
  return (
    <footer className="bg-navy px-8 pt-14 pb-8 border-t border-navyLine">
      <div className="max-w-[1080px] mx-auto">
        <div className="grid gap-10 md:grid-cols-[1.3fr_1fr_1fr] mb-10">
          <div>
            <Link to="/" aria-label="CoreG home" className="inline-block mb-5">
              <img src={logoDark} alt="CoreG" className="h-9" />
            </Link>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-x-5 gap-y-2">
              {FOOTER_NAV.map(([label, path]) => <Link key={path} to={path} className="text-sm text-slateLt hover:text-white">{label}</Link>)}
            </div>
          </div>
          <div>
            <div className="font-mono text-[11px] tracking-[0.12em] text-gold uppercase mb-4">Institutional Governance</div>
            <div className="inline-flex items-center gap-2.5 text-xs text-slateLt">
              ...
              <span>Governed by <span className="text-white/90 font-medium">CoreIdentity</span></span>
            </div>
          </div>
        </div>
